...
int main(int argc, char const *argv[])
{
    ...
    auto checkConnection = [&pointParents, &junctions, &indexes, findParent](const size_t first, const size_t second) {
    ...
    for (size_t i = 0; i < disnaces.size(); ++i) {
        checkConnection(disnaces[i].i, disnaces[i].j);
    }

    long long result = points[indexes[indexes.size() - 1]].x *  points[indexes[indexes.size() - 2]].x;
